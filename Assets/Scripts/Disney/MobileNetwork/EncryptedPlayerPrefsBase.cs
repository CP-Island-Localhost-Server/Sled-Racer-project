...
namespace Disney.MobileNetwork
{
	internal abstract class EncryptedPlayerPrefsBase : IEncryptedPlayerPrefs
	{
		...
		{
			//Discarded unreachable code: IL_0041, IL_004e
		...
		{
			//Discarded unreachable code: IL_0044, IL_0051
		...
		protected void SaveValueEncrypted(string prefsKey, string unencryptedValue)
		{
			...
			string encryptedKey = GetEncryptedKey(keyHash);
			string encryptionKey = GetEncryptionKey(keyHash);
			string value = ((unencryptedValue != null && unencryptedValue.Length != 0) ? AesCipher.Encrypt(unencryptedValue, encryptionKey) : string.Empty);
			if (m_fallbackUnencrypted)
			{
				PlayerPrefs.DeleteKey(prefsKey);
			}
		...
		protected virtual void LoadOrGenerateSecondLevel(string secondaryPrefix, int maxSecondLevelKeys)
		{
			...
			if (m_secondLevel == null)
			{
				...
				int num = random.Next();
				PlayerPrefs.SetInt(masterKeyName, num);
				string masterKey2 = GetMasterKey(num);
				m_secondLevel = new string[maxSecondLevelKeys];
				for (int j = 0; j < m_secondLevel.Length; j++)
				{
					string encryptedKey2 = GetEncryptedKey(secondaryPrefix + (j + 1));
					m_internalKeys[encryptedKey2] = s_stringType;
					m_secondLevel[j] = Guid.NewGuid().ToString();
					string value = AesCipher.Encrypt(m_secondLevel[j], masterKey2);
					PlayerPrefs.SetString(encryptedKey2, value);
				}
				PlayerPrefs.Save();
			}
		}
