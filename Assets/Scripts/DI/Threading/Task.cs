		...
		public event TaskEndedEventHandler TaskEnded
		{
			add
			{
				lock (syncRoot)
				{
					...
					{
						this.taskEnded = (TaskEndedEventHandler)Delegate.Combine(this.taskEnded, value);
					}
				}
			}
			remove
			{
				lock (syncRoot)
				{
					this.taskEnded = (TaskEndedEventHandler)Delegate.Remove(this.taskEnded, value);
				}
			}
		}
		...
		private void RunEnumerator(IEnumerator enumerator)
		{
			...
			do
			{
				...
				else
				{
					...
					{
						Task taskBase2 = Create((Action)delegate
						{
