	...
	public class Channel<T> : IDisposable
	{
		...
		public bool Set(T value, int timeoutInMilliseconds)
		{
			...
			lock (setSyncRoot)
			{
				...
				int num = WaitHandle.WaitAny(new WaitHandle[2] { exitEvent, getEvent }, timeoutInMilliseconds);
				if (num == 258 || num == 0)
				{
					return false;
				}
		...
		public T Get(int timeoutInMilliseconds, T defaultValue)
		{
			...
			lock (getSyncRoot)
			{
				...
				int num = WaitHandle.WaitAny(new WaitHandle[2] { exitEvent, setEvent }, timeoutInMilliseconds);
				if (num == 258 || num == 0)
				{
					return defaultValue;
				}
