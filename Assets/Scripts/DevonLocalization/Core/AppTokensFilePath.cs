namespace DevonLocalization.Core
{
	public class AppTokensFilePath : ILocalizedTokenFilePath
	{
		...
		{
			int num = directoryPath.ToLower().IndexOf("resources");
		...
		{
			return resourcePath + "/" + ((platform != Platform.none) ? (LocalizationPlatform.GetPlatformString(platform) + "/") : string.Empty) + language + resourceExtension;
		}
